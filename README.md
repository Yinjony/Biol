...
题库数据由 CloudBase RDB 直接读写。`server/` 仅保留 Word 文档导入解析服务：
...
Word 导入服务地址在 `miniprogram/config.js` 的 `documentImportApiBaseUrl`。微信开发者工具本地调试时，如果请求被拦，需要在详情设置里勾选“不校验合法域名、web-view（业务域名）、TLS 版本以及 HTTPS 证书”。

当前版本使用 CloudBase RDB 作为题库数据源。
